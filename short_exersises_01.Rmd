...
g1 + coord_fixed(ratio = 1) + theme_bw()
```
...
```{r,echo=F}
g1 + coord_fixed(ratio = -1) + theme_classic() + labs(x=NULL, y=NULL)
```
...
```{r worm,echo=F}
g1 + stat_function(fun=function(x,b) sin(x/b)*4-8, args=list(b=6), n=540, size = 1.5, color = "red") + theme_bw() + labs(title="Стройная ласточка и червяк", x=NULL, y=NULL) +  coord_fixed(ratio = 1)
```